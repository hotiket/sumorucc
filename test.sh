...
assert 30 "{int a, b, c; a=1; b=2; c=3; a=b=c; return a*10;}"
assert 123 "{int x,y,z; x = 2 >= 0; y = (x * x) + (x / x); z = -x * (4 - 5) + y; return x*100 + y*10 + z;}"

assert 72 "{int val001_num ,val002_div ,val003_power; val001_num=3*4*6; val002_div=val001_num/6; val003_power=val002_div*val002_div; return val003_power/2;}"

assert 5 "{int n0, n1; n0 = 5; return n0; n1 = 3; return n1;}"
assert 3 "{return 3; 100;}"

assert 2 "{int a; a = 1; if (a == 0) return 0; return a + 1;}"
assert 1 "{int x; x = 1; if (x == 1) if (2 == 2) return 1; return 0;}"
assert 24 "{int t, u, v; t = 20; u = 0; v = 10; if (t) v = v + 2; if (u) v = 0; return v*2;}"
...
assert 4 "{int a, b; a = 1; {b = a + 1; {return b * b;}} {return a;}}"
assert 0 "{int a; a=0; {{{{{{{{{{{{{{1;}}}}}}}}}}}}}} return a;}"

assert 1 "{;;;;; return 1;}"
assert 100 "{int a; a=100; if (a==0); else; return a;}"

assert 55 "{int sum, i; sum=0; for(i=1; i<=10; i=i+1){sum=sum+i;} return sum;}"
assert 42 "{for(;;) return 42; return 0;}"
assert 4 "{int a; a=1; for(a=2; ; a=3){a=4; return a;} return 0;}"

assert 55 "{int sum, i; sum=0; i=1; while (i<=10) { sum=sum+i; i=i+1; } return sum;}"
assert 36 "{int a, b, c; a=3; while(0) a=0; b=c=0; while(a>0){b=1; while(b<=3){c=c+a*b; b=b+1;} a=a-1;} return c;}"

assert 5 "{int x; x=5; return *&*&x;}"
assert 10 "{int x, y; x=0; y=0; *(&y+1) = 10; return x;}"
assert 20 "{int x, y; x=0; y=0; *(&x-2+1) = 20; return y;}"
assert 2 "{int x, y, z; x=0; y=0; z=0; return &x-&z;}"
assert 4 "{int x; int* y; x=1; y=&x; *y=4; return x;}"
assert 3 "{int x; int *y; x=2; y=&x; return *y+1;}"
assert 21 "{int x, *y, ** z; y=&x; z=&y; **z=21; return x;}"
